...
function FieldLabel({ children, required }: { children: string; required?: boolean }) {
  return (
    <Typography sx={{ fontSize: '12px', fontWeight: 600, color: '#374151', mb: 0.625, letterSpacing: '0.01em' }}>
      {children}
      {required && (
...
export function EditBucketModal({ open, schoolId, bucket, onClose, onEdited }: EditBucketModalProps) {
  const {
    control,
    handleSubmit,
    reset,
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={handleClose} disabled={isSubmitting} size="small">Cancel</Button>
          <Button
            type="submit"
            variant="contained"
            size="small"
