        ...
        {buckets.map((b) => {
          ...
          return (
            <Box
              ...
              onClick={() => {
                if (!disabled) onChange(b.classSectionId);
              }}
              sx={{
                p: 1.1,
                borderRadius: "8px",
            ...
            >
              ...
              {inSlot ? (
                <Typography
                  sx={{ fontSize: "9px", fontWeight: 700, color: "#EF4444", lineHeight: 1.4 }}
                >
                  In slot
                </Typography>
              ) : (
...
export function AddSlotClassModal({
  ...
  const selectedBucket: BucketItem | undefined = prefillBucket
    ? {
        classSectionId: prefillBucket.classSectionId,
        sectionDisplayName: prefillBucket.sectionDisplayName,
        sectionName: prefillBucket.sectionName,
        activeChildrenCount: prefillBucket.activeChildrenCount,
      }
  ...
  useEffect(() => {
    ...
    setDataLoading(true);
    Promise.all([fetchBuckets(schoolId), fetchVolunteers(schoolId)])
      .then(([bks, volRes]) => {
        setBuckets(bks);
  ...
  const maxAllowed = selectedBucket
    ? Math.min(selectedBucket.activeChildrenCount, MAX_CAP)
    : MAX_CAP;
    ...
    >
      ...
      >
        <Box>
          ...
          <Typography sx={{ fontSize: "11px", color: MUTED, mt: 0.25 }}>{slot.slotName}</Typography>
        </Box>
        <IconButton
          size="small"
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <DialogContent sx={{ p: 0, overflowY: "auto" }}>
          ...
          <Box sx={{ px: 3, pt: 2, pb: 1.5, borderBottom: `1px solid ${BORDER}` }}>
            <CompositionPreview bucket={selectedBucket} volunteers={selectedVolunteers} />
          </Box>
