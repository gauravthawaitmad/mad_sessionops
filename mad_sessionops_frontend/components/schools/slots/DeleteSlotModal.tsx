...
export function DeleteSlotModal({ open, schoolId, slot, onClose, onDeleted }: DeleteSlotModalProps) {
  ...
  return (
    ...
    >
      <DialogTitle sx={{ pt: 2.5, pb: 1.5, px: 3, borderBottom: `1px solid ${BORDER}` }}>
        <Typography sx={{ fontSize: '16px', fontWeight: 700, color: '#0F172A' }}>Delete Slot</Typography>
      </DialogTitle>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, py: 1, textAlign: 'center' }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: '13px', color: '#64748B', lineHeight: 1.6 }}>
                ...
                <strong>{slot.slotClassCount}</strong> active class assignment{slot.slotClassCount !== 1 ? 's' : ''}.
                {' '}Remove them first to delete this slot.
              </Typography>
            </Box>
          </Box>
        ) : (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, py: 1, textAlign: 'center' }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: '13px', color: '#64748B', lineHeight: 1.6 }}>
                ...
                <strong>{DAY_LABELS[slot.dayOfWeek]}</strong> slot. This action cannot be undone.
              </Typography>
            </Box>
            {error && (
