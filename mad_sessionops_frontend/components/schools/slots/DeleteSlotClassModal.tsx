...
export function DeleteSlotClassModal({
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setError(
        err?.response?.data?.error?.message ?? err?.message ?? "Failed to remove class assignment."
      );
    } finally {
      setDeleting(false);
    }
        ...
        >
          ...
          <Box>
            ...
            <Typography sx={{ fontSize: "13px", color: "#64748B", lineHeight: 1.6 }}>
              This will remove the <strong>{slotClass?.subjectName}</strong> class assignment from
              this slot. This action cannot be undone.
            </Typography>
          </Box>
