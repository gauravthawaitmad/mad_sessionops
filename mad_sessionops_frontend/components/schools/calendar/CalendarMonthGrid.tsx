...
export function CalendarMonthGrid({
  ...
  const initialMonth = (() => {
    if (today < sessionStart) return { year: sessionStart.getFullYear(), month: sessionStart.getMonth() };
  ...
  const canPrev = new Date(year, month - 1, 1) >= new Date(sessionStart.getFullYear(), sessionStart.getMonth(), 1);
  const canNext = new Date(year, month + 1, 1) <= new Date(sessionEnd.getFullYear(), sessionEnd.getMonth(), 1);
  ...
  while (cur <= last) {
    jumpOptions.push({ label: `${MONTHS[cur.getMonth()]} ${cur.getFullYear()}`, year: cur.getFullYear(), month: cur.getMonth() });
  ...
  return (
    <Box sx={{ px: 4, pt: 2.5, pb: 5 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
        <IconButton size="small" onClick={prevMonth} disabled={!canPrev} sx={{ color: colors.gray[500] }}>
          <ChevronLeft size={18} />
        </IconButton>

        <Typography sx={{ fontSize: '15px', fontWeight: 700, color: colors.gray[800], minWidth: 140, textAlign: 'center' }}>
          {MONTHS[month]} {year}
        </Typography>

        <IconButton size="small" onClick={nextMonth} disabled={!canNext} sx={{ color: colors.gray[500] }}>
        ...
        >
          {jumpOptions.map((o) => (
            <MenuItem key={`${o.year}-${o.month}`} value={`${o.year}-${o.month}`} sx={{ fontSize: '12px' }}>
              {o.label}
            </MenuItem>
          ))}
        </Select>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '1px', mb: '1px' }}>
        {DAY_LABELS.map((d) => (
          <Box key={d} sx={{ textAlign: 'center', py: 0.75 }}>
            <Typography sx={{ fontSize: '11px', fontWeight: 700, color: colors.gray[400], textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              {d}
            </Typography>
          </Box>
        ))}
