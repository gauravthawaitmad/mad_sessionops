...
export function DeleteHolidayModal({ open, holiday, schoolId, onClose, onDeleted }: DeleteHolidayModalProps) {
  ...
  const dateLabel = holiday.startDate === holiday.endDate
  ...
  return (
    <Dialog open={open} onClose={() => { if (!loading) { setError(null); onClose(); } }}
    ...
    >
      <DialogContent sx={{ px: 3, pt: 3, pb: 1 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 1.5 }}>
          <Box sx={{ width: 48, height: 48, borderRadius: '12px', bgcolor: '#FEF2F2', border: '1px solid #FECACA', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Trash2 size={20} strokeWidth={1.75} color="#DC2626" />
          </Box>
          <Box>
            <Typography sx={{ fontSize: '16px', fontWeight: 700, color: colors.gray[900], mb: 0.5 }}>
              Delete holiday?
            </Typography>
      ...
      <DialogActions sx={{ px: 3, py: 2.25, gap: 1, borderTop: `1px solid ${colors.gray[100]}` }}>
        <Button onClick={() => { setError(null); onClose(); }} disabled={loading} variant="outlined"
          sx={{ flex: 1, textTransform: 'none', fontSize: '13px', fontWeight: 500, borderColor: '#E2E8F0', color: colors.gray[700], '&:hover': { borderColor: '#CBD5E1', bgcolor: colors.gray[50] } }}>
          Cancel
        </Button>
        <Button onClick={handleDelete} disabled={loading} variant="contained"
          sx={{ flex: 1, textTransform: 'none', fontSize: '13px', fontWeight: 700, bgcolor: '#DC2626', boxShadow: 'none', '&:hover': { bgcolor: '#B91C1C', boxShadow: 'none' } }}>
