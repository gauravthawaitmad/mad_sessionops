...
function getDurationLabel(start: string, end: string): string | null {
  ...
  const rem = Math.round((e.getTime() - new Date(s.getFullYear(), s.getMonth() + months, s.getDate()).getTime()) / 86_400_000);
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Box sx={{ flex: 1, textAlign: 'center' }}>
          <Typography sx={{ fontSize: '10px', fontWeight: 600, color: colors.gray[400], textTransform: 'uppercase', letterSpacing: '0.06em', mb: 0.25 }}>
            Start
          </Typography>
          <Typography sx={{ fontSize: '13px', fontWeight: 700, color: valid ? colors.primary[700] : colors.gray[400] }}>
            {start ? fmtDate(start) : '—'}
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', px: 1 }}>
          <ArrowRight size={14} strokeWidth={2} color={valid ? colors.primary[500] : colors.gray[300]} />
          {label && (
            <Typography sx={{ fontSize: '10px', fontWeight: 700, color: colors.primary[600], mt: 0.25 }}>
              {label}
            </Typography>
          )}
        </Box>
...
export function SetSessionModal({
  ...
  return (
    ...
    >
      <DialogTitle sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', pt: 2.5, pb: 1.5, px: 3, borderBottom: `1px solid #E2E8F0` }}>
        <Box>
          ...
          <Typography sx={{ fontSize: '11px', color: '#94A3B8', mt: 0.25 }}>
            {academicYearLabel ? `Academic year ${academicYearLabel} · ` : ''}Set the session start and end dates
          </Typography>
        </Box>
        ...
        {defaultsLoading ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Typography sx={{ fontSize: '11px', color: colors.gray[400], mt: -0.75 }}>
              Default values are based on your school's MOU dates. You can change them before saving.
            </Typography>
            ...
            >
              <AlertTriangle size={15} strokeWidth={1.75} color="#D97706" style={{ flexShrink: 0, marginTop: 1 }} />
              <Box>
                ...
                <Typography sx={{ fontSize: '11px', color: '#B45309', lineHeight: '17px' }}>
                  Once saved, the session dates are locked and cannot be edited. Please verify before saving.
                </Typography>
              </Box>
            </Box>
          </Box>
