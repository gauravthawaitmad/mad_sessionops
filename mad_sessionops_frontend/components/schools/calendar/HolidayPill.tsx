...
export function HolidayPill({ holiday, onClick }: HolidayPillProps) {
  ...
  return (
    <Tooltip
      title={
        <Box>
          <Typography sx={{ fontSize: '12px', fontWeight: 600 }}>{holiday.holidayReasonDisplay}</Typography>
          {holiday.holidayDescription && (
            <Typography sx={{ fontSize: '11px', opacity: 0.85, mt: 0.25 }}>{holiday.holidayDescription}</Typography>
          )}
