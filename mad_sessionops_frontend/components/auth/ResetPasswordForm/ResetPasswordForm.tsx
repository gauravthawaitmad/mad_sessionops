...
export function ResetPasswordForm({ token }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await services.auth.resetPassword({ token, newPassword: data.newPassword, confirmPassword: data.confirmPassword });
      setSucceeded(true);
    } catch (err: any) {
      setApiError(err?.message || 'The reset link may be expired. Please request a new one.');
    }
  };
  ...
  return (
    <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
      {apiError && (
        <Alert severity="error" closable onClose={() => setApiError(undefined)} sx={{ mb: 2 }}>
          {apiError}
        </Alert>
      )}
