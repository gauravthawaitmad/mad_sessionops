...
import {
  RefreshCw, UserSearch, Users, Building2, Network, CheckCircle, XCircle, Loader,
} from 'lucide-react';
...
const TABS: TabConfig[] = [
  ...
  { key: 'partner',          label: 'Partners',        Icon: Building2, color: '#7C3AED', bg: '#F5F3FF', statKey: 'partner' },
  { key: 'partner_worknode', label: 'PW Nodes',        Icon: Network,   color: '#059669', bg: '#F0FDF4', statKey: 'partnerWorknode' },
];
...
function TabButton({ tab, isActive, hasRunning, onClick }: TabButtonProps) {
  ...
  return (
    ...
    >
      ...
      {hasRunning && (
        <Box sx={{
          width: 6, height: 6, borderRadius: '50%', bgcolor: '#0284C7',
    ...
    >
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
        <TextField
          ...
          value={email}
          onChange={(e) => { setEmail(e.target.value); setInlineErr(null); }}
...
interface StatBadgeProps { count: number; label: string; color: string }
...
export function DataSyncTab() {
  ...
  const handleSelectRun = useCallback(async (id: number) => {
    ...
    try { setDetail(await fetchSyncRunDetail(id)); }
    catch { setDetail(null); }
    finally { setDetailLoading(false); }
  }, []);

  const handleSyncEntity = useCallback(async (entityKey: SyncEntityType) => {
    setSyncingEntity(entityKey);
    try {
      const result = await triggerEntitySync(entityKey);
      await load();
      const runId =
        entityKey === 'user'             ? result.userRunId :
        entityKey === 'partner'          ? result.partnerRunId :
                                           result.partnerWorknodeRunId;
      if (runId) {
        setSelectedId(runId);
        setDetailLoading(true);
        try { setDetail(await fetchSyncRunDetail(runId)); }
        catch { setDetail(null); }
        finally { setDetailLoading(false); }
      }
    } catch (err: any) {
      const status = err?.response?.status ?? err?.status;
      toast.error(status === 409 ? 'This entity sync is already running.' : 'Failed to start sync.');
    } finally {
  ...
  const handleSyncAll = useCallback(async () => {
    ...
    try {
      ...
      const status = err?.response?.status ?? err?.status;
      toast.error(status === 409 ? 'Another sync is already in progress.' : 'Failed to start sync.');
    } finally {
      setSyncingAll(false);
    }
  }, [load]);
  ...
  const anyRunningEntity = (key: SyncEntityType) =>
    syncingAll || syncingEntity === key || runs.some(r => r.entityType === key && r.status === 'running');
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        {TABS.map(t => (
          <TabButton
            ...
            onClick={() => { setActiveTab(t.key); setSelectedId(null); setDetail(null); }}
          />
        ))}
        ...
        <Button
          ...
          sx={{
            ...
            mb: 0.75, borderColor: BORDER, color: '#475569',
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', p: 2.5, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
          ...
          <Box
            sx={{
              ...
              p: 2, bgcolor: '#FFF', minWidth: 200, flex: '0 0 auto',
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.875 }}>
              ...
              {isActive && (
                <Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center', gap: 0.375 }}>
                  <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: '#0284C7', animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <Typography sx={{ fontSize: '9px', color: '#0284C7', fontWeight: 700 }}>LIVE</Typography>
                </Box>
              )}
            </Box>
            ...
            {stat && (
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                {stat.inactive > 0 && <StatBadge count={stat.inactive} label="inactive" color="#D97706" />}
                {stat.removed  > 0 && <StatBadge count={stat.removed}  label="removed"  color="#DC2626" />}
              </Box>
            )}
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              {lastRun && (
                <Box
                  ...
                  sx={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 0.375, '&:hover': { opacity: 0.75 } }}
                >
                  {lastRun.status === 'success' && <><CheckCircle size={11} color="#16A34A" strokeWidth={2} /><Typography sx={{ fontSize: '10px', color: '#16A34A', fontWeight: 600 }}>success</Typography></>}
                  {lastRun.status === 'failed'  && <><XCircle    size={11} color="#DC2626" strokeWidth={2} /><Typography sx={{ fontSize: '10px', color: '#DC2626', fontWeight: 600 }}>failed</Typography></>}
                  {lastRun.status === 'running' && <><Loader     size={11} color="#0284C7" strokeWidth={2} style={{ animation: 'spin 1.2s linear infinite' }} /><Typography sx={{ fontSize: '10px', color: '#0284C7', fontWeight: 600 }}>running</Typography></>}
                </Box>
              )}
            </Box>
            ...
            <Button
              ...
              disabled={isActive}
              sx={{ fontSize: '11px', textTransform: 'none', alignSelf: 'flex-start', borderColor: tab.color, color: tab.color, '&:hover': { borderColor: tab.color, bgcolor: tab.bg } }}
            >
        ...
        <Box
          sx={{
            border: `1px solid ${BORDER}`, borderRadius: '10px',
            overflow: 'hidden', flex: 1, minHeight: 320,
        ...
        >
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {lastRefreshed && (
                <Typography sx={{ fontSize: '10px', color: '#CBD5E1' }}>
                  · as of {lastRefreshed.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                </Typography>
              )}
            </Box>
            ...
            <Button
              variant="outlined"
              size="small"
              startIcon={statusLoading ? <CircularProgress size={11} color="inherit" /> : <RefreshCw size={11} />}
              onClick={handleRefreshStatus}
              disabled={statusLoading}
              sx={{
                fontSize: '10px', textTransform: 'none', py: 0.25, px: 0.875,
