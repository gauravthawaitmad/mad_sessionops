...
function MetaRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <Box sx={{ display: "flex", gap: 1, py: 0.625, borderBottom: "1px solid #F8FAFC" }}>
      <Typography sx={{ fontSize: "11px", color: "#94A3B8", width: 130, flexShrink: 0 }}>
        {label}
      </Typography>
      <Typography sx={{ fontSize: "11px", color: "#334155", wordBreak: "break-all" }}>
        {value ?? "—"}
      </Typography>
    </Box>
  );
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        {status === "success" && (
          <Typography sx={{ fontSize: "11px", color: "#64748B" }}>
            {pwMode ? (
              fetched > 0 ? (
                `${n(fetched)} rows synced (full sync)`
              ) : (
                "no rows — full sync"
              )
            ) : (
              <>
                ...
                {created > 0 && (
                  <>
                    {" "}
                    · <span style={{ color: "#16A34A" }}>{n(created)} new</span>
                  </>
                )}
...
function SyncedUsersList({ items, isSingle }: { items: SyncedUserLogin[]; isSingle: boolean }) {
  ...
  return (
    <>
      ...
      >
        <Typography
          sx={{ fontSize: "10px", fontWeight: 700, color: "#94A3B8", letterSpacing: "0.08em" }}
        >
      ...
      >
        {visible.map((item, i) => (
          ...
          >
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography
                sx={{ fontSize: "11px", fontWeight: 600, color: "#0F172A", lineHeight: 1.3 }}
              >
                {item.user_name || "—"}
              </Typography>
              <Typography
          ...
          >
            {expanded ? (
              <>
                ...
                <Typography sx={{ fontSize: "10px", color: "#64748B" }}>Show less</Typography>
              </>
            ) : (
              <>
                <ChevronDown size={11} color="#64748B" />
                <Typography sx={{ fontSize: "10px", color: "#64748B" }}>
                  Show all {items.length} users
                </Typography>
              </>
            )}
          </Box>
          ...
          >
            {expanded ? (
              ...
              <>
                <ChevronDown size={11} color="#64748B" />
                <Typography sx={{ fontSize: "10px", color: "#64748B" }}>
                  Show all {items.length} partners
                </Typography>
              </>
            )}
          </Box>
  ...
  return (
    <Box sx={{ p: 2.5, overflowY: "auto", height: "100%" }}>
      ...
      {run.userLogins && run.userLogins.length > 0 && (
        <SyncedUsersList items={run.userLogins} isSingle={run.syncType === "manual_single_user"} />
      )}

      {/* Synced partners list */}
      {run.partnerIds && run.partnerIds.length > 0 && <SyncedPartnersList items={run.partnerIds} />}
    </Box>
  );
}
