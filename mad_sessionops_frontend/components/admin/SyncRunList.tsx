  ...
  return (
    <Box sx={{ overflowY: 'auto', flex: 1 }}>
      {runs.map((run) => {
        ...
        const entityLabel = run.entityType ? (ENTITY_LABEL[run.entityType] ?? run.entityType) : 'all';
          ...
          >
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.625, mb: 0.25 }}>
                ...
                {typeLabel && (
                  <Typography sx={{ fontSize: '9px', color: typeColor, fontWeight: 500, letterSpacing: '0.02em' }}>
                    {typeLabel}
                  </Typography>
                )}
              </Box>
