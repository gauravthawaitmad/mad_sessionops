  ...
  if (!isAdmin) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <Typography sx={{ fontSize: '16px', fontWeight: 600, color: HEADING }}>Access denied</Typography>
  ...
  return (
    <Box sx={{ maxWidth: 720, mx: 'auto', px: 4, pt: 5, pb: 8 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box>
          <Typography sx={{ fontSize: '20px', fontWeight: 700, color: HEADING }}>Academic Years</Typography>
      ...
      {/* List */}
      <Box sx={{ border: `1px solid ${BORDER}`, borderRadius: '10px', overflow: 'hidden', bgcolor: '#fff' }}>
        {loading ? (
          [1, 2, 3].map((i) => (
            <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 2, px: 3, py: 2, borderBottom: `1px solid ${BORDER}` }}>
          ...
          <Box sx={{ py: 6, textAlign: 'center' }}>
            <Typography sx={{ fontSize: '14px', color: MUTED }}>No academic years found.</Typography>
          </Box>
        ) : (
          years.map((year) => (
            <Box
