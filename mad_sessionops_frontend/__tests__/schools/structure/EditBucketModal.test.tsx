...
describe('EditBucketModal — F-M6-6', () => {
  ...
  it('test_prepopulates_current_display_name', () => {
    render(<EditBucketModal open={true} schoolId={580} bucket={MOCK_BUCKET} onClose={noop} onEdited={noop} />);
  ...
  it('test_valid_submit_calls_editBucket', async () => {
    const updated: BucketItem = { ...MOCK_BUCKET, sectionName: 'group_1_renamed', sectionDisplayName: 'Group 1 Renamed' };
    vi.mocked(editBucket).mockResolvedValue(updated);
    const onEdited = vi.fn();

    render(<EditBucketModal open={true} schoolId={580} bucket={MOCK_BUCKET} onClose={noop} onEdited={onEdited} />);
  ...
  it('test_server_409_slug_collision_displays_verbatim', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('A bucket named "Group 2" already exists at this school.')).toBeInTheDocument();
    });
  });
});
