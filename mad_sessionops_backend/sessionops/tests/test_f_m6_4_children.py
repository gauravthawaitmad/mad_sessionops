...
def _make_partner(partner_id: int) -> Partner:
    return Partner.objects.create(
        partner_id=partner_id, partner_name=f"School {partner_id}", converted=True, is_active=True,
    )
...
def _make_school_class(school_id: int, user: User, class_code: str = "5") -> SchoolClass:
    ...
    return SchoolClass.objects.create(
        school_id=school_id, school_academic_year_id=say, class_id_id=cls.class_id, created_by=user,
    )


def _make_bucket(school_id: int, user: User, name: str = "Bucket") -> ClassSection:
    return ClassSection.objects.create(
        school_id=school_id, section_name=name.lower().replace(" ", "_"),
        section_display_name=name, created_by=user,
    )
...
class TestEnrollDecoupled:
    def test_enroll_with_class_only_no_bucket(self):
        ...
        child = enroll_child(
            600, ChildEnrollIn(
                first_name="Solo", last_name="Child", gender="male", age=9,
    ...
    def test_enroll_with_class_and_bucket(self):
        ...
        child = enroll_child(
            601, ChildEnrollIn(
                first_name="Placed", last_name="Child", gender="female", age=9,
                school_class_id=school_class.school_class_id,
...
def test_child_out_current_section_none_when_unassigned(client):
    ...
    resp = client.post(
        ...
        data=json.dumps({
            "first_name": "Unassigned", "last_name": "Kid", "gender": "male", "age": 8,
            "school_class_id": school_class.school_class_id,
        }),
...
def test_child_out_current_section_populated_with_bucket(client):
    ...
    resp = client.post(
        ...
        data=json.dumps({
            "first_name": "Placed", "last_name": "Kid", "gender": "female", "age": 8,
            "school_class_id": school_class.school_class_id,
            "class_section_id": bucket.class_section_id,
        }),
        content_type="application/json",
...
class TestListChildrenUnassignedFilter:
    def test_unassigned_filter_returns_only_bucketless_children(self):
        ...
        enroll_child(604, ChildEnrollIn(
            first_name="NoBucket", last_name="A", gender="male", age=8,
            school_class_id=school_class.school_class_id,
        ), user)
        enroll_child(604, ChildEnrollIn(
            first_name="WithBucket", last_name="B", gender="male", age=8,
            school_class_id=school_class.school_class_id,
            class_section_id=bucket.class_section_id,
        ), user)
    ...
    def test_unassigned_filter_takes_precedence_over_section_id(self):
        ...
        enroll_child(605, ChildEnrollIn(
            first_name="Solo", last_name="A", gender="male", age=8,
            school_class_id=school_class.school_class_id,
        ), user)
...
class TestEditChildSchoolClassId:
    def test_edit_without_school_class_id_leaves_class_unchanged(self):
        ...
        child = enroll_child(606, ChildEnrollIn(
            first_name="Stay", last_name="Same", gender="male", age=8,
            school_class_id=school_class.school_class_id,
        ), user)
