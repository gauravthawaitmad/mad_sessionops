...
def _co():
    uid = next(_UID)
    return User.objects.create(
        user_login=f"co{uid}@t.com",
        user_display_name=f"CO{uid}",
        email=f"co{uid}@t.com",
...
def _session(
    school: Partner, co: User, start=date(2026, 7, 1), end=date(2027, 4, 30)
...
def test_create_holiday_overlapping_active_holiday_returns_409():
    ...
    create_holiday(
        school.partner_id,
        {**PAYLOAD, "start_date": date(2026, 8, 10), "end_date": date(2026, 8, 20)},
        co,
    )
    # Overlaps
    with pytest.raises(ConflictError, match="overlaps"):
        create_holiday(
            school.partner_id,
            {**PAYLOAD, "start_date": date(2026, 8, 15), "end_date": date(2026, 8, 25)},
            co,
        )
...
def test_create_holiday_optional_fields_can_be_null():
    ...
    _session(school, co)
    h = create_holiday(
        school.partner_id, {**PAYLOAD, "holiday_description": None, "remarks": None}, co
    )
...
def test_edit_holiday_date_change_triggers_overlap_check():
    ...
    create_holiday(
        school.partner_id,
        {**PAYLOAD, "start_date": date(2026, 8, 1), "end_date": date(2026, 8, 5)},
        co,
    )
    h2 = create_holiday(
        school.partner_id,
        {**PAYLOAD, "start_date": date(2026, 9, 1), "end_date": date(2026, 9, 5)},
        co,
    )
    ...
    with pytest.raises(ConflictError, match="overlaps"):
        edit_holiday(
            h2.school_holiday_id,
            {"start_date": date(2026, 8, 3), "end_date": date(2026, 8, 10)},
            co,
        )
...
def test_list_holidays_windowed_query():
    ...
    create_holiday(
        school.partner_id,
        {**PAYLOAD, "start_date": date(2026, 10, 1), "end_date": date(2026, 10, 5)},
        co,
    )

    result = list_holidays(
        school.partner_id, start_date=date(2026, 7, 1), end_date=date(2026, 9, 30)
    )
    assert len(result) == 1
