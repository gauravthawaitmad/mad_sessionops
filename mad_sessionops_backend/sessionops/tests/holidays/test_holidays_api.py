...
def test_list_holidays_returns_403_for_other_co(client):
    _year()
    co1 = _user("CO Full Time")
    co2 = _user("CO Full Time")
    school = _school(co1)
...
def test_post_holiday_409_on_overlap(client):
    ...
    resp = client.post(
        f"/api/schools/{school.partner_id}/holidays/",
        data=json.dumps(
            {"holiday_reason": "holidays", "start_date": "2026-08-15", "end_date": "2026-08-25"}
        ),
        content_type="application/json",
        **_h(co),
    )
...
def test_post_holiday_400_outside_session_window(client):
    ...
    resp = client.post(
        f"/api/schools/{school.partner_id}/holidays/",
        data=json.dumps(
            {"holiday_reason": "holidays", "start_date": "2025-01-01", "end_date": "2025-01-05"}
        ),
        content_type="application/json",
        **_h(co),
    )
...
def test_delete_holiday_returns_204(client):
    ...
    resp = client.delete(
        f"/api/schools/{school.partner_id}/holidays/{h.school_holiday_id}/", **_h(co)
    )
