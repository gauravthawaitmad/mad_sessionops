...
def _execute_user_sync(run: SyncRun) -> None:
    ...
    try:
        ...
        if total > 0 and cursor_end == run.updated_after:
            logger.warning("%s cursor_end unchanged — all row timestamps equal or missing (boundary overlap?), held at %s", tag, run.updated_after)
...
def _execute_partner_sync(run: SyncRun) -> None:
    ...
    try:
        ...
        cursor_end = _compute_cursor_end(rows, "partner_updated_date", "updated_at", fallback=run.updated_after)
        if total > 0 and cursor_end == run.updated_after:
            logger.warning("%s cursor_end unchanged — all row timestamps equal or missing (boundary overlap?), held at %s", tag, run.updated_after)
...
def _trigger_entities(entity_types: list[str], triggered_by: User, global_lock: bool) -> dict[str, int]:
    if global_lock:
    ...
    logger.info("trigger: starting manual sync for %s — triggered_by=%s", entity_types, triggered_by_name)
...
def trigger_manual_sync(triggered_by: User) -> dict:
    ...
    result = _trigger_entities(["user", "partner", "partner_worknode"], triggered_by, global_lock=True)
    return {
