...
## Second real PR run (2026-07-21) — backend + frontend `test` failures, now resolved

- **Real bug:** `AuthService.request_password_reset` (`sessionops/services/auth_service.py`) raised `AuthenticationError` for an unknown email and for a user with no allowed role, contradicting its own docstring ("returns silently... prevents email enumeration") and breaking `test_f_m1_1_password_auth.py::test_unknown_email_is_silent`. Both branches now return silently instead of raising.
- **Dead test suite, deleted (not fixed):** `sessionops/tests/api/test_auth.py` (19+ failures — 404s and shape mismatches) tested the Google OAuth endpoints in `sessionops/api/auth.py` (`/google/callback`, `/refresh`, `/logout`, `/me`). Per `docs/milestones/M1.md` (lines 65, 303), this F01a Google OAuth code is intentionally paused until M5 and deliberately not mounted in `routes.py` — the actual M1 auth flow is password-based (`auth_api.py` / `auth_service.py`). This test file predates that pivot and was never removed when Google OAuth got parked. Deleted per user decision (2026-07-21) rather than wiring up paused functionality ahead of schedule.
- **Test bug, fixed:** `mad_sessionops_frontend/__tests__/admin/DataSyncTab.test.tsx` — resolves Open Question 3 above. The per-entity sync buttons now get an explicit `syncLabel` per tab (`components/admin/DataSyncTab.tsx`) instead of deriving button text from the compact tab label (`"PW Nodes"` → `"Sync pw nodes"` didn't match `/sync partner worknodes/i`, and only one tab's button is ever in the DOM at a time). Tests updated to switch tabs before asserting each entity's button. Full frontend suite (130 tests) verified green locally.

## Third real PR run (2026-07-21) — backend `test` failure, now resolved

- **Test bug, fixed:** `test_f_m2_6_children.py::TestListChildren::test_status_all_includes_inactive` deactivated its fixture child via a raw `Child.objects...update(is_active=False, removed=True)`, but the real `deactivate_child` service (`services/children/deactivate.py:33`) only ever sets `is_active=False` on the `Child` row itself — `removed=True` updates are scoped to the `ChildClass`/`ChildClassSection`/etc. junction rows, not `Child`. `removed=True` on `Child` means hard-removed, which `list_children`'s `status="all"` branch correctly excludes (matching `test_status_all_returns_both` in `test_f_m2_10_children.py`, which drives the same scenario through the real service and passes). Fixture now only sets `is_active=False`, matching how deactivation actually leaves the row.

## Blockers
- **`CODECOV_TOKEN`** remains a manual, out-of-band step (Settings → Secrets → Actions) — not resolvable from code.
- **Frontend lint debt (141 errors)** (Open Question 2 above) blocks flipping frontend lint to fully blocking in CI/pre-push — currently informational/non-blocking by design, not by oversight. The DataSyncTab test failures (Open Question 3) are resolved as of 2026-07-21 — `npm run test` no longer needs the non-blocking fallback for this reason.
